  ...
  try {
    ...
    const samplePets = [
      ...
      },
      {
        ...
        age: 3,
        age_unit: 'years',
        description: 'Charlie is a curious and playful Beagle who loves to explore. He has a gentle temperament and gets along well with everyone.',
    ...
    for (const pet of samplePets) {
      await db.run(
        `INSERT INTO pets (name, category_id, breed, age, age_unit, description, image_url, shelter_id, created_by, adoption_status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'available')`,
        [pet.name, pet.category_id, pet.breed, pet.age, pet.age_unit, pet.description, pet.image_url, pet.shelter_id, pet.created_by]
      );
    }
    ...
    // Verify data
    const petCount = await db.get('SELECT COUNT(*) as count FROM pets');
    const userCount = await db.get('SELECT COUNT(*) as count FROM users');
    const categoryCount = await db.get('SELECT COUNT(*) as count FROM pet_categories');
